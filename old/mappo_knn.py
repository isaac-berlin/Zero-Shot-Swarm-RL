...
def stack_global_state(env) -> np.ndarray:
    """
    Global state: location of all agents and items concatenated.
    This remains independent of the per-agent kNN observation.
    """
    state = []
...
class SharedActor(nn.Module):
    """
    MLP-based actor for kNN vector observations.
    Input: obs_dim vector per agent (e.g., 12).
    """
...
class CentralCritic(nn.Module):
    """
    Centralized V(s) where s is global state (concat of all agent & item positions).
    """
...
@dataclass
class Transition:
    obs: np.ndarray            # per-agent kNN obs (vector)
    state: np.ndarray          # global state (concat positions)
    action: int
    logp: float
    value: float
    reward: float
...
class MAPPO:
    def __init__(
        ...
        self.critic = CentralCritic(state_dim, hidden_critic).to(device)
...
def train_mappo(
    ...
    # infer obs_dim from new kNN observation
    dummy_obs, _ = env.reset()
    obs_sample = dummy_obs[agent_order[0]]          # e.g., shape (12,)
    obs_dim = obs_sample.shape[0]

    # infer state_dim from global state function
    sample_state = stack_global_state(env)          # e.g., 2*num_agents + 2*num_items
    ...
    writer = SummaryWriter(log_dir="runs/mappo_knn")
...
if __name__ == "__main__":
    ...
    torch.save(algo.actor.state_dict(), "mappo_knn_actor.pth")